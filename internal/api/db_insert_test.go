...
func TestParseProperty(t *testing.T) {
	...
	property, err := parseProperty(defined, "title", "hello")
	assert.Nil(t, err)
	...
	property, err = parseProperty(defined, "rich_text", "hello")
	assert.Nil(t, err)
	...
	property, err = parseProperty(defined, "number", "1")
	assert.Nil(t, err)
	...
	property, err = parseProperty(defined, "multi_select", "hello,world")
	assert.Nil(t, err)
	...
	property, err = parseProperty(defined, "date", "2020-01-01")
	assert.Nil(t, err)
	assert.Equal(t, notionapi.Date(time.Date(2020, 1, 1, 0, 0, 0, 0, time.UTC)), *property.(notionapi.DateProperty).Date.Start)
	property, err = parseProperty(defined, "date", "x")
	assert.NotNil(t, err)
	assert.Equal(t, "invalid date x", err.Error())
	assert.Empty(t, property)

	property, err = parseProperty(defined, "formula", "1+1")
	assert.NotNil(t, err)
	assert.Empty(t, property)

	property, err = parseProperty(defined, "relation", "id")
	assert.Nil(t, err)
	...
	property, err = parseProperty(defined, "rollup", "")
	assert.NotNil(t, err)
	assert.Empty(t, property)

	property, err = parseProperty(defined, "people", "foo,bar")
	assert.Nil(t, err)
	...
	property, err = parseProperty(defined, "files", "hello")
	assert.NotNil(t, err)
	assert.Empty(t, property)

	property, err = parseProperty(defined, "checkbox", "true")
	assert.Nil(t, err)
	assert.Equal(t, true, property.(notionapi.CheckboxProperty).Checkbox)
	property, err = parseProperty(defined, "checkbox", "t")
	assert.Nil(t, err)
	assert.Equal(t, true, property.(notionapi.CheckboxProperty).Checkbox)
	property, err = parseProperty(defined, "checkbox", "yes")
	assert.Nil(t, err)
	assert.Equal(t, true, property.(notionapi.CheckboxProperty).Checkbox)
	property, err = parseProperty(defined, "checkbox", "y")
	assert.Nil(t, err)
	assert.Equal(t, true, property.(notionapi.CheckboxProperty).Checkbox)
	property, err = parseProperty(defined, "checkbox", "")
	assert.Nil(t, err)
	assert.Equal(t, false, property.(notionapi.CheckboxProperty).Checkbox)

	property, err = parseProperty(defined, "url", "hello")
	assert.Nil(t, err)
	assert.Equal(t, "hello", property.(notionapi.URLProperty).URL)

	property, err = parseProperty(defined, "email", "hello")
	assert.Nil(t, err)
	assert.Equal(t, "hello", property.(notionapi.EmailProperty).Email)

	property, err = parseProperty(defined, "phone_number", "hello")
	assert.Nil(t, err)
	assert.Equal(t, "hello", property.(notionapi.PhoneNumberProperty).PhoneNumber)

	property, err = parseProperty(defined, "invalid", "hello")
	assert.NotNil(t, err)
	assert.Empty(t, property)

	property, err = parseProperty(defined, "unknown", "hello")
	assert.NotNil(t, err)
	assert.Empty(t, property)
}
